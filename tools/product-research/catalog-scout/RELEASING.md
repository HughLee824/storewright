...
From the repository root, use the shared multi-tool release entry point:
...
Alternatively, pass `--push` to let the shared script perform that final push. This triggers the
release workflow, so use it only when the release should be published immediately.

## Adding another tool

Register the tool in the repository's `release-tools.toml`. Each entry owns its directory,
display name, tag prefix, version file matching/replacement rules, changelog, managed files,
lock/update commands, and quality checks. Tools therefore share release mechanics without sharing
version numbers or tags. Text manifests such as `pyproject.toml` and `package.json` are supported
through the configured version pattern and replacement.
