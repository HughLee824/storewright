...
Shop Scout 是面向已授权店铺数据的商品图片筛选与本地归档工具。它先用商品列表图调用 SerpApi Google Lens 精确匹配，再决定是否读取并保存完整商品详情；按类目最多保留指定数量的合格商品，并在店铺淘汰率显著过高时提前停止，减少 API、页面访问和存储消耗。
...
```bash
uv run shop-scout browser login
uv run shop-scout browser diagnose
```

完全离线 Mock E2E：

```bash
uv run shop-scout run \
...
```bash
uv run shop-scout resume --run-id <uuid> --confirm-authorized
uv run shop-scout report --run-id <uuid>
uv run shop-scout rebuild-archives --run-id <uuid>
uv run shop-scout review list --run-id <uuid>
```
