...
from shop_scout.adapters.taobao import TaobaoAdapter, canonical_product_url, extract_item_id
from shop_scout.browser.navigator import BrowserUseNavigator
...
from shop_scout.domain.models import ProductDetail, ProductRef, ShopIdentity
from shop_scout.exceptions import ManualActionRequiredError, ShopScoutError
...
class PlaywrightCatalogBackend(CatalogBackend):
    ...
    async def collect_pool(self, shop: ShopIdentity, max_items: int) -> list[ProductRef]:
        ...
        if not direct_ready:
            ...
            if not navigation.success:
                raise ShopScoutError(f"NAVIGATION_FAILED: {navigation.reason}")
        ...
        if initial_state != PageKind.PRODUCT_LISTING:
            raise ShopScoutError("NAVIGATION_FAILED: product listing was not reached")
    ...
    async def fetch_image_url(self, image_url: str, referer_url: str) -> tuple[bytes, str]:
        ...
        response = await self.image_client.get(
            image_url,
            headers={"Referer": referer_url, "User-Agent": "shop-scout/0.1"},
        )
